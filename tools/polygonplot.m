...
function polygonplot(data,opt_axes,opt_lines,opt_area)
    ...
    if(~isfield(opt_axes,'Ticks'))
        opt_axes.Ticks = linspace(data_min,data_max,20);
    ...
    if(isfield(opt_axes,'Labels'))
        ...
        for lid = 1:1:N
            if xlbl(lid) < 0 
                text(xlbl(lid)-0.1,ylbl(lid),opt_axes.Labels{lid},'FontSize',8);
            else
                text(xlbl(lid),ylbl(lid),opt_axes.Labels{lid},'FontSize',8);
            end
        end
    end
    ...
    if(M == 0)    % Only mean
        ...
        if(labels)
            for n = 1:1:nlines
                for t = 1:1:N
                    if X(t, n) <0
                        text(X(t,n) - 1,Y(t,n),num2str(data(t,n)),'FontSize',4);    
                    else
                        text(X(t,n),Y(t,n),num2str(data(t,n)),'FontSize',4);
                    end
                end
            end
        end
        ...
        if(labels)
            for n = 1:1:nlines
                for t = 1:1:N
                    if X(t, n) <0
                        text(X(t,n) - 1,Y(t,n),num2str(data_mean(t,n)),'FontSize',4);    
                    else                    
                        text(X(t,n),Y(t,n),num2str(data_mean(t,n)),'FontSize',4);
                    end
                end
            end
        end
