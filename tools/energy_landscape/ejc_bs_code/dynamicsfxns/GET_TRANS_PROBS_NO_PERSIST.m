function [transitionProbability,transProbEndGivenStart] = GET_TRANS_PROBS_NO_PERSIST(kClusterAssignments,subjInd,start)
...
for N = 1:nobs %nobs
    ...
    elseif start == 2
        transProbEndGivenStart(N,:,:) = squeeze(numTransitions(N,:,:)) ./ repmat(sum(squeeze(numTransitions(N,:,:)),1)',[1 numClusters])';
    else
        transProbEndGivenStart(N,:,:) = squeeze(numTransitions(N,:,:)) ./ repmat(repmat(sum(sum(squeeze(numTransitions(N,:,:)),1)),[1 numClusters])', [1 numClusters]);
    end
end
