function [clusterNames,reorderClusters,clusterNamesSort,net8angle] = NAME_CLUSTERS_ANGLE8(centroids, net)
...
elseif nparc == 86
    ...
    network8labels=fs86_to_yeo;
elseif nparc == 200 || nparc == 232
    load('data/sch232_to_yeo.csv'); 
    network8labels=sch232_to_yeo;
elseif nparc == 100 || nparc == 116
    if net == 0
        load('data/sch116_to_yeo.csv'); 
        network8labels=sch116_to_yeo;
    elseif net == 1
        load('data/sch116_to_yeo_17.csv');
        network8labels=sch116_to_yeo_17;
    end
elseif nparc == 400 || nparc == 454
    load('data/sch454_to_yeo.csv');
    network8labels=sch454_to_yeo;
elseif nparc == 462
    load('data/Lausanne_463_subnetworks.mat');

    network8labels=subnetworks_reorder;
    network8labels(463)=[];
elseif nparc == 461 %ls463 with brainstem (last region) and region 14 removed (artefacts)
    load('data/Lausanne_463_subnetworks.mat');
    network8labels=subnetworks_reorder;
    network8labels([14 463])=[];
else
    if net == 0
        load('data/sch116_to_yeo.csv');
        tmp = 8*ones(1, nparc);
        tmp(1:length(sch116_to_yeo)) = sch116_to_yeo;
        network8labels=tmp;
        
    elseif net == 1
        load('data/sch116_to_yeo_17.csv');
        tmp = 18*ones(1, nparc);
        tmp(1:length(sch116_to_yeo_17)) = sch116_to_yeo_17;
        network8labels=tmp;
    end
end

network8labels=network8labels(1:nparc);
network8labels=reshape(network8labels,nparc,1);
if net == 0
    YeoNetNames = {'VIS+', 'SOM+', 'DAT+', 'VAT+', 'LIM+', 'FPN+', 'DMN+','SC+', 'VIS-', 'SOM-', 'DAT-', 'VAT-', 'LIM-', 'FPN-', 'DMN-', 'SC-'};
    numNets = 8;
elseif net == 1
    YeoNetNames = {'VISCent+', 'VISPer+', 'SOMa+','SOMb+', 'DATa+', 'DATb+', 'VATa+', 'VATb+', 'LIMb+', 'LIMa+', 'ContA+', 'ContB+', 'ContC+', 'DMNa+', 'DMNb+', 'DMNc+', 'TP+', 'SC+', 'VISCent-', 'VISPer-', 'SOMa-','SOMb-', 'DATa-', 'DATb-', 'VATa-', 'VATb-', 'LIMb-', 'LIMa-', 'ContA-', 'ContB-', 'ContC-', 'DMNa-', 'DMNb-', 'DMNc-', 'TP-', 'SC-'};
    numNets = 18;
end
...
net8angle = zeros(numClusters,numNets*2);

for K = 1:numClusters
    for B = 1:(numNets*2)
        net8angle(K,B) = dot(centroids(:,K),binaryNetVectors(:,B))...
...
for K = 1:numClusters
    ind = find(net8angle(K,:) == max(net8angle(K,:)));    %cos(0) = 1 so need max not min (like for E.D.)
