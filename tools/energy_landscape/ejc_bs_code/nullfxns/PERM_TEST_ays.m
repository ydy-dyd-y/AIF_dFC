function [pvals_twotail,pval] = PERM_TEST_ays(A,B,nperms)
    ...
    % INPUT:
    % A and B: NxSxK stack of SxK transition matrices for N subjects and two conditions, A and B
    ...
    [nobs, numSystems, numClusters] = size(A);
    permutationIndex = rand(nobs,nperms) > 0.5; % preallocate uniform random swap indices
    expectedA_gt_B = zeros(nperms,numSystems,numClusters); % difference under null hypothesis 
    for P = 1:nperms
