...
m     = n(end);
xm    = spm_vec(x{m});
J     = cell(1,size(V{m},2));  % num_par만큼 길이의 cell
...
%%
    ...
    %----------------------------------------------------------------------
    if isempty(xm)
        print('no arguments to differentiate w.r.t')
        J = sparse(length(f),0);
        
    % or there are no arguments to differentiate
    %----------------------------------------------------------------------
    elseif isempty(f)
        print('no arguments to differentiate')
    ...
    for i = 1:length(J)
        ...
        fi    = spm_diff(f,xi{:},n(1:end - 1),V);   %  sparse array of length = num_par
        J{i}  = spm_dfdx(fi,f0{1},dx);  % f0{1} is array of length = num_par
        p     = p & isnumeric(J{i});
    end
