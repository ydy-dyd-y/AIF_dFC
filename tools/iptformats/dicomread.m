...
readPixels = true;
[attrs, settings] = builtin('_dicomparse',fileDetails.name, ...
                               fileDetails.bytes, ...
                               machineEndian, ...
                               readPixels, ...
                               dictionaryFile, ...
...
elseif (metadata.BitsAllocated == 8)
    ...
    % Unpack the data.
    metadata.InstanceData = builtin('_bitparse', metadata.InstanceData, ...
...
for p = 1:segmentOffsets(1)
    ...
    [pixCodes(p,:), prevWarning] = ...
        builtin('_dicom_decode_rle_segment', metadata.InstanceData(start:stop), ...
    ...
    for p = 1:numOverlays
        ...
        if (metadata.OverlayBitPosition{p} == 0)
            ...
            if ((rows ~= imageRows) || (cols ~= imageCols))
                ...
                tmp = builtin('_tobits', metadata.OverlayData{attrOverlayCount});
...
if (metadata.OverlayBitPosition{1} == 0)
    ...
    tmp = builtin('_tobits', metadata.OverlayData{1});
