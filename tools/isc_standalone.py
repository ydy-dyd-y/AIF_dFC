...
def isfc(data, targets=None, pairwise=False, summary_statistic=None,
    ...
    targets, t_n_TRs, t_n_voxels, t_n_subejcts, symmetric = (   # (231, 147, 54), 231, 147, 54, True
    ...
    # Compute ISFCs using leave-one-out approach
        ...
        data = np.rollaxis(data, 2, 0)  # (subj, tp, nodes) : (54, 231, 147)
        targets = np.rollaxis(targets, 2, 0)  # (subj, tp, nodes) : (54, 231, 147)
        ...
        isfcs = [np.corrcoef(np.ascontiguousarray(subject.T),
                             np.ascontiguousarray(mean(
                                    ...
                                    axis=0).T))[:data.shape[2],
                                                data.shape[2]:]
                 for s, subject in enumerate(data)] # isfcs is list and len = Nsubj
    ...
    if vectorize_isfcs and symmetric:
        isfcs, iscs = squareform_isfc(isfcs) # isfcs.shape = (subj x edges) = (54, 10731)
        return isfcs, iscs
    else:
        return isfcs
...
def squareform_isfc(isfcs, iscs=None):
    ...
    if not type(iscs) == np.ndarray and isfcs.shape[-2] == isfcs.shape[-1]:  # isfcs가 (nsubj x node1 x node2)인 경우 node1 == node2를 뜻함.
        if isfcs.ndim == 2:
            isfcs = isfcs[np.newaxis, ...]
        if isfcs.ndim == 3:
...
def circular_timeshift(data, random_state=None):
    """
    Parameters
    ----------
    data : ndarray (n_TRs x n_voxels x n_subjects)
    ...
    # Get a random set of shifts based on number of TRs,
    shifts = prng.choice(np.arange(n_TRs), size=n_subjects, replace=True)
        
    shifted_data = []
    for subject, shift in zip(data, shifts):
        shifted_data.append(np.concatenate(
                                (subject[-shift:, :],
                                 subject[:-shift, :])))  # 길이 Nsubj의 리스트이고, 각 요소는 Time x ROI의 array
    shifted_data = np.dstack(shifted_data)
    return shifted_data
