...
for task_seq = 1:2 % 1 is loss task, 2 is reward task
    for dec_param = 2:3 % 1: alpha,beta, 2: beta, 3: alpha
        ...
        avg_LL_3o_L1_params = [];
        avg_prob_3o_L1_params = [];
        avg_LL_3o_L3_params = [];
        avg_prob_3o_L3_params = [];
        ...
        try
            ...
            parfor si = 1:length(data)  % specify different true risk-seeking values (prior = 2)
                ...
                if task_seq == 1
                    if dec_param == 1
                        DCM1.field = {'alpha', 'beta', 'la', 'omega'}; % parameter (field) names to optimise
                    elseif dec_param == 2
                        DCM1.field = {'beta', 'la', 'omega'};
                    else
                        DCM1.field = {'alpha', 'la', 'omega'};
                    end
                else
                    if dec_param == 1
                        DCM1.field = {'alpha', 'beta', 'rs', 'omega'}; % parameter (field) names to optimise
                    elseif dec_param == 2
                        DCM1.field = {'beta', 'rs', 'omega'};
                    else
                        DCM1.field = {'alpha', 'rs', 'omega'};
                    end
                end
                ...
                F_3o_L1_params = [F_3o_L1_params DCM1.F];% Get free energies for each participant's model
                
                GCM_3o_L1   = [GCM_3o_L1;{DCM1}]; % Save DCM for each participant
                ...
                elseif dec_param == 2
                    ...
                    [MDP_best(1:N).omega] = deal(DCM1.posterior(3));
                else
                    [MDP_best(1:N).alpha] = deal(DCM1.posterior(1));
                    [MDP_best(1:N).omega] = deal(DCM1.posterior(3));
                end
                ...
                avg_LL_3o_L1 = L/(size(MDP_best,2)*2);
                
                avg_LL_3o_L1_params = [avg_LL_3o_L1_params; avg_LL_3o_L1];
                
                avg_prob_3o_L1 = total_prob/(size(MDP_best,2)*2);
                
                avg_prob_3o_L1_params = [avg_prob_3o_L1_params; avg_prob_3o_L1];
                
                % Store true and estimated parameters to assess recoverability
                
                Sim_params_3o_L1 = [Sim_params_3o_L1; DCM1.posterior];% Get posteriors
                
                GMDP_3o_L1 = [GMDP_3o_L1;{MDP_best}];
            ...
            for si = 1:length(data)  % specify different true risk-seeking values (prior = 2)
                ...
                %MDP_temp = spm_MDP_VB_X(MDP_temp); % <- this line seems to be useless
        ...
        if dec_param == 1
            Estimated_alpha_3o_L1 = Sim_params_3o_L1(:,1);
            Estimated_beta_3o_L1 = Sim_params_3o_L1(:,2);
            if task_seq == 1
                Estimated_la_3o_L1 = Sim_params_3o_L1(:,3);
            else
                Estimated_rs_3o_L1 = Sim_params_3o_L1(:,3);
            end
            Estimated_omega_3o_L1 = Sim_params_3o_L1(:,4);
        elseif dec_param == 2
            Estimated_beta_3o_L1 = Sim_params_3o_L1(:,1);
            if task_seq == 1
                Estimated_la_3o_L1 = Sim_params_3o_L1(:,2);
            else
                Estimated_rs_3o_L1 = Sim_params_3o_L1(:,2);
            end
            Estimated_omega_3o_L1 = Sim_params_3o_L1(:,3);
        else
            Estimated_alpha_3o_L1 = Sim_params_3o_L1(:,1);
            if task_seq == 1
                Estimated_la_3o_L1 = Sim_params_3o_L1(:,2);
        ...
        if tmp == 1
            parfor si = 1:length(data)  % specify different true risk-seeking values (prior = 2)
                ...
                if task_seq == 1
                    if dec_param == 1
                        DCM3.field = {'alpha', 'beta', 'la', 'omega'};
                    elseif dec_param == 2
                        DCM3.field = {'beta', 'la', 'omega'};
                    else
                        DCM3.field = {'alpha', 'la', 'omega'};
                    end
                else
                    if dec_param == 1
                        DCM3.field = {'alpha', 'beta', 'rs', 'omega'};
                    elseif dec_param == 2
                        DCM3.field = {'beta', 'rs', 'omega'};
                    else
                        DCM3.field = {'alpha', 'rs', 'omega'};
                    end
                end
                ...
                F_3o_L3_params = [F_3o_L3_params DCM3.F]; % Get free energies for each participant's model
                
                GCM_3o_L3   = [GCM_3o_L3;{DCM3}]; % Save DCM for each participant
                ...
                elseif dec_param == 2
                    ...
                    [MDP_best(1:N).omega] = deal(DCM3.posterior(3));
                else
                    [MDP_best(1:N).alpha] = deal(DCM3.posterior(1));
                    [MDP_best(1:N).omega] = deal(DCM3.posterior(3));
                end
                ...
                avg_LL_3o_L3 = L/(size(MDP_best,2)*2);
                avg_LL_3o_L3_params = [avg_LL_3o_L3_params; avg_LL_3o_L3];
                avg_prob_3o_L3 = total_prob/(size(MDP_best,2)*2);
                avg_prob_3o_L3_params = [avg_prob_3o_L3_params; avg_prob_3o_L3];
                
                % Store true and estimated parameters to assess recoverability
                
                Sim_params_3o_L3 = [Sim_params_3o_L3; DCM3.posterior];% Get posteriors
                GMDP_3o_L3 = [GMDP_3o_L3;{MDP_best}];
        ...
        % Separately store true and simulated parameters
        if dec_param == 1
            Estimated_alpha_3o_L3 = Sim_params_3o_L3(:,1);
            Estimated_beta_3o_L3 = Sim_params_3o_L3(:,2);
            if task_seq == 1
                Estimated_la_3o_L3 = Sim_params_3o_L3(:,3);
            else
                Estimated_rs_3o_L3 = Sim_params_3o_L3(:,3);
            end
            Estimated_omega_3o_L3 = Sim_params_3o_L3(:,4);
        elseif dec_param == 2
            Estimated_beta_3o_L3 = Sim_params_3o_L3(:,1);
            if task_seq == 1
                Estimated_la_3o_L3 = Sim_params_3o_L3(:,2);
            else
                Estimated_rs_3o_L3 = Sim_params_3o_L3(:,2);
            end
            Estimated_omega_3o_L3 = Sim_params_3o_L3(:,3);
        else
            Estimated_alpha_3o_L3 = Sim_params_3o_L3(:,1);
            if task_seq == 1
                Estimated_la_3o_L3 = Sim_params_3o_L3(:,2);
        ...
        F_3o_L3_params = F_3o_L3_params'; % Convert free energies to column vectors
        
        average_LL_3o_L3 = mean(avg_LL_3o_L3_params);
        average_action_probability_3o_L3 = mean(avg_prob_3o_L3_params);
        
        if dec_param == 1
            if task_seq == 1
                save('/u6/pmdd/behav/loss_est_3o.mat', 'Estimated_alpha_3o_L3', ...
                    'Estimated_beta_3o_L3', ...
                    'Estimated_la_3o_L3', 'Estimated_omega_3o_L3')
                
                save('/u6/pmdd/behav/loss_result_3o.mat', 'F_3o_L3_params', 'GCM_3o_L3', ...
                    'GMDP_3o_L3', 'average_action_probability_3o_L3',...
                    'average_LL_3o_L3', 'avg_LL_3o_L3_params', 'avg_prob_3o_L3_params')
            else
                save('/u6/pmdd/behav/reward_est_3o.mat', 'Estimated_alpha_3o_L3', ...
                    'Estimated_beta_3o_L3', ...
                    'Estimated_rs_3o_L1', 'Estimated_rs_3o_L3', 'Estimated_omega_3o_L3')
                
                save('/u6/pmdd/behav/reward_result_3o.mat', 'F_3o_L3_params', 'GCM_3o_L3', ...
                    'GMDP_3o_L3', 'average_action_probability_3o_L3',...
                    'average_LL_3o_L3', 'avg_LL_3o_L3_params', 'avg_prob_3o_L3_params')
            end
        elseif dec_param == 2
            if task_seq == 1
                save('/u6/pmdd/behav/loss_est_3o_be.mat', 'Estimated_beta_3o_L3', ...
                    'Estimated_la_3o_L3', 'Estimated_omega_3o_L3')
                
                save('/u6/pmdd/behav/loss_result_3o_be.mat', 'F_3o_L3_params', 'GCM_3o_L3', ...
                    'GMDP_3o_L3', 'average_action_probability_3o_L3',...
                    'average_LL_3o_L3', 'avg_LL_3o_L3_params', 'avg_prob_3o_L3_params')
            else
                save('/u6/pmdd/behav/reward_est_3o_be.mat', 'Estimated_beta_3o_L3', ...
                    'Estimated_rs_3o_L1', 'Estimated_rs_3o_L3', 'Estimated_omega_3o_L3')
                
                save('/u6/pmdd/behav/reward_result_3o_be.mat', 'F_3o_L3_params', 'GCM_3o_L3', ...
                    'GMDP_3o_L3', 'average_action_probability_3o_L3',...
                    'average_LL_3o_L3', 'avg_LL_3o_L3_params', 'avg_prob_3o_L3_params')
            end
        else
            if task_seq == 1
                save('/u6/pmdd/behav/loss_est_3o_al.mat', 'Estimated_alpha_3o_L3', ...
                    'Estimated_la_3o_L3', 'Estimated_omega_3o_L3')
                
                save('/u6/pmdd/behav/loss_result_3o_al.mat', 'F_3o_L3_params', 'GCM_3o_L3', ...
                    'GMDP_3o_L3', 'average_action_probability_3o_L3',...
                    'average_LL_3o_L3', 'avg_LL_3o_L3_params', 'avg_prob_3o_L3_params')
            else
                save('/u6/pmdd/behav/reward_est_3o_al.mat', 'Estimated_alpha_3o_L3', ...
                    'Estimated_rs_3o_L1', 'Estimated_rs_3o_L3', 'Estimated_omega_3o_L3')
                
                save('/u6/pmdd/behav/reward_result_3o_al.mat', 'F_3o_L3_params', 'GCM_3o_L3', ...
                    'GMDP_3o_L3', 'average_action_probability_3o_L3',...
                    'average_LL_3o_L3', 'avg_LL_3o_L3_params', 'avg_prob_3o_L3_params')
            end
        end
