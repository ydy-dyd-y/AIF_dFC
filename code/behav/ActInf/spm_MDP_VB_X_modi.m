...
for m = 1:size(MDP,1)
    ...
    if isfield(MDP,'un')
        un_prev = [];
        for t = 1:size(MDP(m).un, 2)/Ni
            un_prev = [un_prev, MDP(m).un(:,t*Ni)];
        end
    end
    ...
    for f = 1:Nf(m)
        ...
        errn{m,f} = zeros(Ni,Ns(m,f),1,1,Np(m));
        x{m,f}  = zeros(Ns(m,f),T,Np(m))      + 1/Ns(m,f);
        X{m,f}  = repmat(D{m,f},1,1);
        qln{m,f} = zeros(Ni,Ns(m,f),1,1,Np(m));
        for k = 1:Np(m)
            x{m,f}(:,1,k) = D{m,f};
        end
    end
        ...
        if ~HMM || T == t
            ...
            if t < T
                ...
                for i = 1:Np(m)
                    ...
                    try, Pu(sub{:}) = Pu(sub{:}) + un_prev(i,t);
                    catch, Pu(sub{:}) = Pu(sub{:}) + u{m}(i,t);
                    end
                end
                ...
                try
                    ...
                    MDP(m).u(:,t) = spm_ind2sub(Nu(m,:),ind);  %[2 x 1]
                end
    ...
    for f = 1:Nf(m)
        ...
        for i = 1:T
            for k = 1:Np(m)
                ...
                Vn{m,f}(:,:,1:T,i) = Vn{m,f}(:,:,1:T,i) + vn{m,f}(:,:,1:T,i,k)*u{m}(k,i);  % u is calculated by(line983): u{m}(p{m},t)  = qu
                %ERRn{m,f}(:,:,1:T,i) = ERRn{m,f}(:,:,1:T,i) + errn{m,f}(:,:,1:T,i,k)*u{m}(k,i);
            end
        end
    end
