...
%%
for task_seq = 1:2 % 1 is loss task, 2 is reward task
    for dec_param = 1:3 % 1: alpha,beta, 2: beta, 3: alpha
        ...
        omega = 0.5; % By default we here set this to 1 (indicating no forgetting,
        
        beta = 1.5; % By default this is set to 1, but try increasing its value
        ...
        avg_LL_4_L1_params = [];
        avg_prob_4_L1_params = [];
        avg_LL_4_L3_params = [];
        avg_prob_4_L3_params = [];
        ...
        try
            ...
            parfor si = 1:length(data)  % specify different true risk-seeking values (prior = 2)
                ...
                if task_seq == 1
                    if dec_param == 1
                        DCM1.field = {'alpha', 'beta', 'la', 'eta', 'omega'}; % parameter (field) names to optimise
                    elseif dec_param == 2
                        DCM1.field = {'beta', 'la', 'eta', 'omega'};
                    else
                        DCM1.field = {'alpha', 'la', 'eta', 'omega'};
                    end
                else
                    if dec_param == 1
                        DCM1.field = {'alpha', 'beta', 'rs', 'eta', 'omega'}; % parameter (field) names to optimise
                    elseif dec_param == 2
                        DCM1.field = {'beta', 'rs', 'eta', 'omega'};
                    else
                        DCM1.field = {'alpha', 'rs', 'eta', 'omega'};
                    end
                end
                ...
                F_4_L1_params = [F_4_L1_params DCM1.F];% Get free energies for each participant's model
                
                GCM_4_L1   = [GCM_4_L1;{DCM1}]; % Save DCM for each participant
                ...
                if dec_param == 1
                    ...
                    [MDP_best(1:N).omega] = deal(DCM1.posterior(5));
                elseif dec_param == 2
                    [MDP_best(1:N).beta] = deal(DCM1.posterior(1));
                    [MDP_best(1:N).eta] = deal(DCM1.posterior(3));
                    [MDP_best(1:N).omega] = deal(DCM1.posterior(4));
                else
                    [MDP_best(1:N).alpha] = deal(DCM1.posterior(1));
                    [MDP_best(1:N).eta] = deal(DCM1.posterior(3));
                    [MDP_best(1:N).omega] = deal(DCM1.posterior(4));
                ...
                MDP_best   = spm_MDP_VB_X_modi(MDP_best); % run model with best parameter values
                ...
                avg_LL_4_L1 = L/(size(MDP_best,2)*2);
                
                avg_LL_4_L1_params = [avg_LL_4_L1_params; avg_LL_4_L1];
                
                avg_prob_4_L1 = total_prob/(size(MDP_best,2)*2);
                
                avg_prob_4_L1_params = [avg_prob_4_L1_params; avg_prob_4_L1];
                
                % Store true and estimated parameters to assess recoverability
                
                Sim_params_4_L1 = [Sim_params_4_L1; DCM1.posterior];% Get posteriors
                
                GMDP_4_L1 = [GMDP_4_L1;{MDP_best}];
        ...
        if dec_param == 1
            Estimated_alpha_4_L1 = Sim_params_4_L1(:,1);
            Estimated_beta_4_L1 = Sim_params_4_L1(:,2);
            if task_seq == 1
                Estimated_la_4_L1 = Sim_params_4_L1(:,3);
            else
                Estimated_rs_4_L1 = Sim_params_4_L1(:,3);
            end
            Estimated_eta_4_L1 = Sim_params_4_L1(:,4);
            Estimated_omega_4_L1 = Sim_params_4_L1(:,5);
        elseif dec_param == 2
            Estimated_beta_4_L1 = Sim_params_4_L1(:,1);
            if task_seq == 1
                Estimated_la_4_L1 = Sim_params_4_L1(:,2);
            else
                Estimated_rs_4_L1 = Sim_params_4_L1(:,2);
            end
            Estimated_eta_4_L1 = Sim_params_4_L1(:,3);
            Estimated_omega_4_L1 = Sim_params_4_L1(:,4);
        else
            Estimated_alpha_4_L1 = Sim_params_4_L1(:,1);
            if task_seq == 1
                Estimated_la_4_L1 = Sim_params_4_L1(:,2);
        ...
        if tmp == 1
            parfor si = 1:length(data)  % specify different true risk-seeking values (prior = 2)
                ...
                if task_seq == 1
                    if dec_param == 1
                        DCM3.field = {'alpha', 'beta', 'la', 'eta', 'omega'};
                    elseif dec_param == 2
                        DCM3.field = {'beta', 'la', 'eta', 'omega'};
                    else
                        DCM3.field = {'alpha', 'la', 'eta', 'omega'};
                    end
                else
                    if dec_param == 1
                        DCM3.field = {'alpha', 'beta', 'rs', 'eta', 'omega'};
                    elseif dec_param == 2
                        DCM3.field = {'beta', 'rs', 'eta', 'omega'};
                    else
                        DCM3.field = {'alpha', 'rs', 'eta', 'omega'};
                    end
                end
                ...
                F_4_L3_params = [F_4_L3_params DCM3.F]; % Get free energies for each participant's model
                
                GCM_4_L3   = [GCM_4_L3;{DCM3}]; % Save DCM for each participant
                ...
                if dec_param == 1
                    ...
                    [MDP_best(1:N).omega] = deal(DCM3.posterior(5));
                elseif dec_param == 2
                    [MDP_best(1:N).beta] = deal(DCM3.posterior(1));
                    [MDP_best(1:N).eta] = deal(DCM3.posterior(3));
                    [MDP_best(1:N).omega] = deal(DCM3.posterior(4));
                else
                    [MDP_best(1:N).alpha] = deal(DCM3.posterior(1));
                    [MDP_best(1:N).eta] = deal(DCM3.posterior(3));
                    [MDP_best(1:N).omega] = deal(DCM3.posterior(4));
                ...
                avg_LL_4_L3 = L/(size(MDP_best,2)*2);
                avg_LL_4_L3_params = [avg_LL_4_L3_params; avg_LL_4_L3];
                avg_prob_4_L3 = total_prob/(size(MDP_best,2)*2);
                avg_prob_4_L3_params = [avg_prob_4_L3_params; avg_prob_4_L3];
                
                % Store true and estimated parameters to assess recoverability
                
                Sim_params_4_L3 = [Sim_params_4_L3; DCM3.posterior];% Get posteriors
                GMDP_4_L3 = [GMDP_4_L3;{MDP_best}];
        ...
        % Separately store true and simulated parameters
        if dec_param == 1
            Estimated_alpha_4_L3 = Sim_params_4_L3(:,1);
            Estimated_beta_4_L3 = Sim_params_4_L3(:,2);
            if task_seq == 1
                Estimated_la_4_L3 = Sim_params_4_L3(:,3);
            else
                Estimated_rs_4_L3 = Sim_params_4_L3(:,3);
            end
            Estimated_eta_4_L3 = Sim_params_4_L3(:,4);
            Estimated_omega_4_L3 = Sim_params_4_L3(:,5);
        elseif dec_param == 2
            Estimated_beta_4_L3 = Sim_params_4_L3(:,1);
            if task_seq == 1
                Estimated_la_4_L3 = Sim_params_4_L3(:,2);
            else
                Estimated_rs_4_L3 = Sim_params_4_L3(:,2);
            end
            Estimated_eta_4_L3 = Sim_params_4_L3(:,3);
            Estimated_omega_4_L3 = Sim_params_4_L3(:,4);
        else
            Estimated_alpha_4_L3 = Sim_params_4_L3(:,1);
            if task_seq == 1
                Estimated_la_4_L3 = Sim_params_4_L3(:,2);
        ...
        %F_4_L1_params = F_4_L1_params';
        F_4_L3_params = F_4_L3_params'; % Convert free energies to column vectors
        
        %[alpha,exp_r,xp,pxp,bor] = spm_BMS([F_4_L1_params F_4_L3_params]);
        ...
        % provide a probability of each model being the best-fit model.
        %--------------------------------------------------------------------------
        
        % We can also calculate the average probability and log-likelihood (LL) of the
        % actions under each model:
        
        %average_LL_4_L1 = mean(avg_LL_4_L1_params);
        %average_action_probability_4_L1 = mean(avg_prob_4_L1_params);
        average_LL_4_L3 = mean(avg_LL_4_L3_params);
        average_action_probability_4_L3 = mean(avg_prob_4_L3_params);
        
        disp(' ');
        %fprintf('Average log-likelihood under the d model: %.2g\n',average_LL_4_L1);
        %fprintf('Average action probability under the 4-parameter d model: %.2g\n',average_action_probability_4_L1);
        disp(' ');
        fprintf('Average log-likelihood under the a, d model: %.2g\n',average_LL_4_L3);
        fprintf('Average action probability under the 4-parameter a, d model: %.2g\n',average_action_probability_4_L3);
        disp(' ');

        %%
        if dec_param == 1
            if task_seq == 1
                save(['/u6/pmdd/behav/loss_est_4.mat'], 'Estimated_alpha_4_L3', ...
                    'Estimated_la_4_L3', 'Estimated_eta_4_L3', ...
                    'Estimated_beta_4_L3', ...
                    'Estimated_omega_4_L3')
                
                save(['/u6/pmdd/behav/loss_result_4.mat'], 'F_4_L3_params', 'GCM_4_L3', ...
                    'GMDP_4_L3', 'average_action_probability_4_L3', ...
                    'average_LL_4_L3', 'avg_LL_4_L3_params', 'avg_prob_4_L3_params')
            else
                save(['/u6/pmdd/behav/reward_est_4.mat'], 'Estimated_alpha_4_L3', ...
                    'Estimated_rs_4_L3', 'Estimated_eta_4_L3', ...
                    'Estimated_beta_4_L3', ...
                    'Estimated_omega_4_L3')
                
                save(['/u6/pmdd/behav/reward_result_4.mat'], 'F_4_L3_params', 'GCM_4_L3', ...
                    'GMDP_4_L3', 'average_action_probability_4_L3', ...
                    'average_LL_4_L3', 'avg_LL_4_L3_params', 'avg_prob_4_L3_params')
            end
        elseif dec_param == 2
            if task_seq == 1
                save(['/u6/pmdd/behav/loss_est_4_be.mat'], 'Estimated_beta_4_L3', ...
                    'Estimated_la_4_L3', 'Estimated_eta_4_L3', ...
                    'Estimated_omega_4_L3')
                
                save(['/u6/pmdd/behav/loss_result_4_be.mat'], 'F_4_L3_params', 'GCM_4_L3', ...
                    'GMDP_4_L3', 'average_action_probability_4_L3', ...
                    'average_LL_4_L3', 'avg_LL_4_L3_params', 'avg_prob_4_L3_params')
            else
                save(['/u6/pmdd/behav/reward_est_4_be.mat'], 'Estimated_beta_4_L3', ...
                    'Estimated_rs_4_L3', 'Estimated_eta_4_L3', ...
                    'Estimated_omega_4_L3')
                
                save(['/u6/pmdd/behav/reward_result_4_be.mat'], 'F_4_L3_params', 'GCM_4_L3', ...
                    'GMDP_4_L3', 'average_action_probability_4_L3',...
                    'average_LL_4_L3', 'avg_LL_4_L3_params', 'avg_prob_4_L3_params')
            end
        else
            if task_seq == 1
                save(['/u6/pmdd/behav/loss_est_4_al.mat'], 'Estimated_alpha_4_L3', ...
                    'Estimated_la_4_L3', 'Estimated_eta_4_L3', ...
                    'Estimated_omega_4_L3')
                
                save(['/u6/pmdd/behav/loss_result_4_al.mat'], 'F_4_L3_params', 'GCM_4_L3', ...
                    'GMDP_4_L3', 'average_action_probability_4_L3',...
                    'average_LL_4_L3', 'avg_LL_4_L3_params', 'avg_prob_4_L3_params')
            else
                save(['/u6/pmdd/behav/reward_est_4_al.mat'], 'Estimated_alpha_4_L3', ...
                    'Estimated_rs_4_L3', 'Estimated_eta_4_L3', ...
                    'Estimated_omega_4_L3')
                
                save(['/u6/pmdd/behav/reward_result_4_al.mat'], 'F_4_L3_params', 'GCM_4_L3', ...
                    'GMDP_4_L3', 'average_action_probability_4_L3',...
                    'average_LL_4_L3', 'avg_LL_4_L3_params', 'avg_prob_4_L3_params')
            end
        end
        cd(str1)
        clearvars('-except', 'task_seq', 'root_path', 'str1');
        close all      % These commands clear the workspace and close any figures
        clc
