...
parser.add_argument("-i", "--input", type=int, nargs=1, help="regional level: 0=node, 1=edge, 2=state, 3=system", default=0)
...
if region_level == 0:
    ...
    Y_values = neuro_con_data_interp # (3465, Nedges, 54)
elif region_level == 2:
    ...
    #scipy.io.savemat(os.path.join(root_path, 'results', 'graph_met'), {'system_con_data_interp':system_con_data_interp})
...
for di in range(1, num_m+1):
    ...
    if EV_name in locals():
        ...
        params_mass = np.zeros([Nregions, Nsubj, X.shape[2]+1])
        std_error_mass = np.zeros([Nregions, Nsubj, X.shape[2]+1])
        t_values_mass = np.zeros([Nregions, Nsubj, X.shape[2]+1])
        p_values_mass = np.zeros([Nregions, Nsubj, X.shape[2]+1])
        for roi in range(Nregions):
            ...
            for si in range(Nsubj):
                ...
                for vi in range(X.shape[2]):
                    ...
                    Xx[:, vi] = x_minmax
                Y = np.squeeze(Y_values[:,roi,si])  # [3465 x 1]
                Xx1 = Xx[114:1715,:]
                Y1 = Y[114:1715]
                Xx2 = Xx[1814:3415,:]
                Y2 = Y[1814:3415]
                ...
                params_mass[roi, si, :] = params_tmp
                std_error_mass[roi, si, :] = std_error_tmp
                t_values_mass[roi, si, :] = t_values_tmp
                p_values_mass[roi, si, :] = p_values_tmp
        scipy.io.savemat(os.path.join(result_path,'act_'+region_level_label[region_level]+'_DS'+str(format(di, '02'))+'_'+task_label[task_type]+'.mat'), 
